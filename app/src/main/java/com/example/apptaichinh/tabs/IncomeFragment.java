...
import com.example.apptaichinh.CalendarActivity;
import com.example.apptaichinh.Card_TK;
import com.example.apptaichinh.DatabaseHelper;
import com.example.apptaichinh.MainActivity;
import com.example.apptaichinh.Profile;
import com.example.apptaichinh.R;
import com.google.android.material.button.MaterialButton;
import com.google.android.material.snackbar.Snackbar;
import java.text.SimpleDateFormat;
...
public class IncomeFragment extends Fragment {
    ...
    // Đặt ngày hiện tại theo định dạng dd/MM/yyyy cho người dùng
    private void setCurrentDate() {
        Calendar calendar = Calendar.getInstance();
        SimpleDateFormat dateFormat = new SimpleDateFormat("dd/MM/yyyy", new Locale("vi", "VN"));
        String currentDate = dateFormat.format(calendar.getTime());
    ...
    // Tải các danh mục thu nhập từ cơ sở dữ liệu vào GridLayout
    private void loadCategories(GridLayout categoryGrid) {
        SQLiteDatabase db = databaseHelper.getReadableDatabase();
    ...
    // Lưu khoản thu vào cơ sở dữ liệu với ngày tháng chuẩn
    private void saveIncome(String date, String notes, String amount, String category) {
        ...
        ContentValues values = new ContentValues();
        values.put(DatabaseHelper.COLUMN_DATE, formattedDate);  // Lưu ngày theo định dạng chuẩn
        values.put(DatabaseHelper.COLUMN_NOTES, notes);
        values.put(DatabaseHelper.COLUMN_AMOUNT, Double.parseDouble(amount));
        values.put(DatabaseHelper.COLUMN_CATEGORY, category);
